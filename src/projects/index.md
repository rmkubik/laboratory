...
{{ it.context
      ...
      .sort((a, b) => forceDateToTimeZone(b.transformations.matter.projectDate, currTz).getTime() - forceDateToTimeZone(a.transformations.matter.projectDate, currTz).getTime())
      .forEach(function(file){ 
}}
  <li><a href="{{= file.slug }}">{{= file.transformations.matter.title }}</a> - {{=new Intl.DateTimeFormat("en-US", {year:"numeric",month:"short"}).format(forceDateToTimeZone(file.transformations.matter.projectDate, currTz))}} - {{= file.transformations.matter.description }}</li>
{{ }) }}
</ul>
